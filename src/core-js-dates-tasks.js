...
function getTime(date) {
  const string = date.toTimeString().split(' ');
  return string[0];
}
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  ...
  const result =
    (new Date(dateEnd).getTime() - new Date(dateStart).getTime()) / msPerDay;
...
function getWeekNumberByDate(date) {
  ...
  const dStart = new Date(year, 0, 1);
  return Math.ceil(((date - dStart) / 1000 / 60 / 60 / 24 + 1) / 7);
}
...
function getNextFridayThe13th(date) {
  ...
  for (let i = 0; i <= 11; i += 1) {
    ...
    if (day.getDay() === 0) {
      month13 = day.getMonth();
      break;
    }
  }
  return new Date(year, month13, 13);
}
...
function isLeapYear(date) {
  const year = date.getFullYear();
  const febr29 = new Date(year, 1, 29);
  if (febr29.getDate() === 29) {
    return true;
  }
  return false;
}
